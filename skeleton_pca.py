...
def get_pictures_by_name(name='Ariel Sharon'):
	...
	print("names:", set(str(s) for s in lfw_people.target_names))
...
def PCA(X, k):
	...
	indexes = np.argsort(eigenvals)[::-1]
	sorted_eigenvalues = eigenvals[indexes]
	sorted_eigenvectors = eigenvecs[:, indexes]
	U = sorted_eigenvectors[:, :k].T
	S = sorted_eigenvalues[:k]
	return U, S
